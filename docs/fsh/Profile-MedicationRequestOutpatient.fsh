...
* subject -> "pharmacy (med).units,med.units"
* authoredOn -> "pharmacy (med).unitsPerDose,med.unitsPerDose"
* requester -> "pharmacy (med).currentProvider,pharmacy (med).doseStart,pharmacy (med).doseStop,pharmacy (med).duration,med.currentProvider,med.doseStart,med.doseStop,med.duration"
* dosageInstruction.doseAndRate.doseQuantity.value -> "pharmacy (med).dose [m],med.dose [m]"
* dosageInstruction.doseAndRate.doseQuantity.unit -> "pharmacy (med).dose [m],med.dose [m]"
* dispenseRequest.validityPeriod.end -> "pharmacy (med).expires,med.expires"
* dispenseRequest.quantity.value -> "pharmacy (med).schedule,med.schedule"
* dispenseRequest.expectedSupplyDuration -> "pharmacy (med).daysSupply,med.daysSupply"
* dosageInstruction.doseAndRate.extension[http://hl7.org/fhir/StructureDefinition/originalText].valueString -> "pharmacy (med).dose [m],med.dose [m]"
* identifier -> "pharmacy (med).dose,med.dose"
* dosageInstruction.route.coding.code -> "pharmacy (med).dose [m],med.dose [m]"