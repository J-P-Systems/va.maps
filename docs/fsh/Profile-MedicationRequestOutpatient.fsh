...
* extension[http://va.gov/fhir/StructureDefinition/medicationrequest-pharmacyOrderStatus].valueCoding.code -> "Medication.PharmacyStatus\nMedication.VAStatus"
* medicationCodeableConcept.text -> "Medication.DrugProduct\nMedication.CMOP"
* medicationCodeableConcept.coding.code -> "Medication.DrugProduct\nMedication.CMOP"
* authoredOn -> "Medication.VAStartDate\nMedication.FillQuantity"
...
* dispenseRequest.validityPeriod.end -> "Medication.ToTime\nMedication.Expires"
* extension[http://va.gov/fhir/StructureDefinition/medicationrequest-cancelDate].valueDateTime -> "Medication.ToTime\nMedication.DiscontinuedDateTime"
...
* dosageInstruction.doseAndRate.extension[http://hl7.org/fhir/StructureDefinition/originalText].valueString -> "Medication.DosageSteps\nMedication.Route\nMedication.DoseQuantity\nMedication.DoseUoM\nMedication.Duration\nMedication.Frequency\nMedication.TextInstruction\nMedication.Conjunction\nMedication.Noun\nMedication.UnitsPerDose\nMedication.Verb"
* identifier.value -> "Medication.PrescriptionNumber\nMedication.FillDate"
* dosageInstruction.route.coding.code -> "Medication.DosageSteps\nMedication.Route\nMedication.DoseQuantity\nMedication.DoseUoM\nMedication.Duration\nMedication.Frequency\nMedication.TextInstruction\nMedication.Conjunction\nMedication.Noun\nMedication.UnitsPerDose\nMedication.Verb"
...
* authoredOn -> "med.unitsPerDose\npharmacy (med).unitsPerDose"
* requester -> "med.currentProvider (>200-.01)\nmed.currentProvider (>200-8932.1)\nmed.doseStart\nmed.doseStop\nmed.duration\npharmacy (med).currentProvider (>200-.01)\npharmacy (med).currentProvider (>200-8932.1)\npharmacy (med).doseStart\npharmacy (med).doseStop\npharmacy (med).duration"
* dosageInstruction.doseAndRate.doseQuantity.value -> "med.dose [m]\npharmacy (med).dose [m]"
* dosageInstruction.doseAndRate.doseQuantity.unit -> "med.dose [m]\npharmacy (med).dose [m]"
* dispenseRequest.validityPeriod.end -> "med.expires\npharmacy (med).expires"
* dispenseRequest.quantity.value -> "med.schedule\npharmacy (med).schedule"
* dispenseRequest.expectedSupplyDuration -> "med.daysSupply\npharmacy (med).daysSupply"
* dosageInstruction.doseAndRate.extension[http://hl7.org/fhir/StructureDefinition/originalText].valueString -> "med.dose [m]\npharmacy (med).dose [m]"
* identifier.value -> "med.dose\npharmacy (med).dose"
* dosageInstruction.route.coding.code -> "med.dose [m]\npharmacy (med).dose [m]"